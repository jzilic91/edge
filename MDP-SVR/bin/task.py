...
class Task:
	...
	def execute(cls):
		if cls._execute:
			MdpLogger.write_log("Task " + cls._name + " cannot be executed since it is already been executed!")
			return ExecutionErrorCode.EXE_NOK
		elif cls._in_edges:
			MdpLogger.write_log("Task " + cls._name + " cannot be executed since other dependable tasks " + str(cls._in_edges) + " are not executed!")
			return ExecutionErrorCode.EXE_NOK
		else:
			...
			for task in cls._out_edges:
				if not task.remove_in_edge(cls):
					...
					MdpLogger.write_log("Invalid task dependancy during application execution!")
					return ExecutionErrorCode.EXE_NOK
	...
	def remove_in_edge(cls, executed_task):
		if executed_task in cls._in_edges:
			...
			MdpLogger.write_log("Task " + cls._name + " is not depended on executed task " + executed_task.get_name() + "!")
			return False
