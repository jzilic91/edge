...
class MobileApplication:
	...
	def __init__(self, name, delay_dict):
		...
		# MdpLogger.write_log("======================= INITIALIZE TASK DEPENDENCIES =======================")
	...
	def run(cls):
		if not cls._running:
			cls._running = True
			# MdpLogger.write_log(cls._name + " application is RUNNING!\n")
		#else:
			# MdpLogger.write_log(cls._name + " application is ALREADY running!\n")
	...
	def get_ready_tasks(cls):
		...
		if not cls._running:
			MdpLogger.write_log("You have to run " + cls._name + " application before you start execution!")
			return False
		...
		if not ready_tasks:
			for task in cls._delay_dict:
				if not task.is_executed():
					MdpLogger.write_log(cls._name + " application is not executed entirely but it should!")
					MdpLogger.write_log("There are no ready tasks but not all tasks are tagged as executed!")
					MdpLogger.write_log("Error in application execution order!")
					MdpLogger.write_log("Task " + task.get_name() + " is not executed but there are no ready tasks for execution!")
					return False
	...
	def print_task_config(cls):
		# MdpLogger.write_log("********************** TASK CONFIGURATION **********************")
		for task in cls._delay_dict:
			task.print_system()
	...
	def print_task_exe_status(cls):
		...
		# MdpLogger.write_log("********************** EXECUTION STATUS **********************")
		...
		for task in cls._delay_dict:
			...
			if task.is_executed():
				...
				# MdpLogger.write_log(print_text)
			# if task was not executed then print text according to execution status
			#else:
				# MdpLogger.write_log(task.get_name() + "(" + str(task.is_offloadable()) + ")" + " is NO_EXE")
