...
class OffloadingDecisionEngine(ABC):
	...
	def __evaluate_params(cls, mobile_device, edge_servers, cloud_dc, network):
		...
		if not isinstance(cloud_dc, OffloadingSite):
			Logger.write_log("Cloud data center should be OffloadingSite object instance in ODE class!")
			return False

		if not edge_servers:
			Logger.write_log("Edge servers should not be empty in ODE class!")
			return False

		for edge in edge_servers:
			if not isinstance(edge, OffloadingSite):
				Logger.write_log("Edge servers should be OffloadingSite object instance in ODE class!")
				return False

		if not (cloud_dc.get_offloading_site_code() == OffloadingSiteCode.CLOUD_DATA_CENTER):
			Logger.write_log("Cloud data center should be configured as cloud data center in ODE class!")
			return False
		...
		for edge in edge_servers:
			if not (edge.get_offloading_site_code() in [OffloadingSiteCode.EDGE_DATABASE_SERVER, OffloadingSiteCode.EDGE_COMPUTATIONAL_INTENSIVE_SERVER, \
				...
				Logger.write_log("Edge server should be configured as edge server in ODE class!")
				return False
	...
	def __compute_complete_energy_consumption(cls, uplink_time, execution_time, downlink_time, current_node, previous_node):
		...
		if current_node.get_offloading_site_code() == OffloadingSiteCode.MOBILE_DEVICE and\
			...
			# print("Energy execution consumption!!!! Execution time: " + str(execution_time))
		...
		# use case: mobile device -> Edge/Cloud servers
			...
			previous_node.get_offloading_site_code() == OffloadingSiteCode.MOBILE_DEVICE:
			# print("Energy uplink consumption!!!! Uplink time: " + str(uplink_time) + ", idle time: " + str(execution_time) + ", downlink time: " + str(downlink_time))
		...
		# use case: Cloud/Edge -> Cloud/Edge (successive tasks executed on the same node)
			...
			# print("Energy consumption during remote execution!!!! Execution time: " + str(execution_time) + ", downlink time: " + str(downlink_time))
			execution_time_power = cls.__compute_idle_energy_consumption(execution_time)
			downlink_time_power = cls.__compute_downlink_energy_consumption(downlink_time)
		...
		# use case: Cloud/Edge -> Cloud/Edge (successive tasks executed on the different nodes)	
			...
			previous_node.get_offloading_site_code() != OffloadingSiteCode.MOBILE_DEVICE and\
			current_node.get_name() != previous_node.get_name():
			# print("Energy consumption during remote execution!!!! Uplink + Execution time: " + str(uplink_time + execution_time) + ", downlink time: " + str(downlink_time))
		...
		# use case: Cloud/Edge -> mobile device
			...
			previous_node.get_offloading_site_code() != OffloadingSiteCode.MOBILE_DEVICE:
			# print("Energy downlink consumption!!!! Downlink time: " + str(uplink_time) + ", execution time: " + str(execution_time))
			execution_time_power = cls.__compute_execution_energy_consumption(execution_time)
	...
	def __compute_failure_cost(cls, uplink_time, execution_time, downlink_time, uplink_energy, execution_energy, downlink_energy, candidate_node, previous_node):
		...
		if uplink_time == 0.0 and downlink_time == 0.0:
			...
			# Logger.write_log("Failure cost only for local server execution!")
			...
			# Logger.write_log("Task completion cost: " + str(execution_time) + "s\n")
			...
			# Logger.write_log("Task energy: " + str(execution_energy) + "J\n")
			# Logger.write_log("Total reward: -" + str(task_overall_reward))
		...
		if choice:
			...
			if downlink_time != 0.0:
				...
				if choice:
					...
					# Logger.write_log("Failure cost only for downlink failure!")
					# Logger.write_log("Uplink time: " + str(uplink_time) + "s")
					# Logger.write_log("Execution time: " + str(execution_time) + "s")
					# Logger.write_log("Downlink time: " + str(downlink_time) + "s")
					# Logger.write_log("Task completion cost: " + str(task_time) + "s\n")

					# Logger.write_log("Uplink energy: " + str(uplink_energy) + "J")
					# Logger.write_log("Execution energy: " + str(execution_energy) + "J")
					# Logger.write_log("Downlink energy: " + str(downlink_energy) + "J")
					# Logger.write_log("Task energy: " + str(task_energy) + "J\n")
					# Logger.write_log("Total reward: -" + str(task_overall_reward))
			...
			# Logger.write_log("Failure cost only for uplink failure!")
			# Logger.write_log("Uplink time: " + str(uplink_time) + "s")
			# Logger.write_log("Execution time: " + str(execution_time) + "s")
			# Logger.write_log("Downlink time: " + str(downlink_time) + "s")
			# Logger.write_log("Task completion cost: " + str(uplink_time) + "s\n")

			# Logger.write_log("Uplink energy: " + str(uplink_energy) + "J")
			# Logger.write_log("Execution energy: " + str(execution_energy) + "J")
			# Logger.write_log("Downlink energy: " + str(downlink_energy) + "J")
			# Logger.write_log("Task energy: " + str(uplink_energy) + "J\n")
			# Logger.write_log("Total reward: -" + str(task_overall_reward))
		...
		# use case 4: offloading server failure
			...
			# Logger.write_log("Failure cost only for offloading server failure!")
			# Logger.write_log("Uplink time: " + str(uplink_time) + "s")
			# Logger.write_log("Execution time: " + str(execution_time) + "s")
			# Logger.write_log("Downlink time: " + str(downlink_time) + "s")
			# Logger.write_log("Task completion cost: " + str(task_time) + "s\n")
	...
	def __compute_overall_task_reward(cls, task_completion_time_reward, task_energy_consumption_reward):
		Logger.write_log("Time Reward: " + str(cls._w_f_time_completion * task_completion_time_reward) + ", Energy Reward: " + str(cls._w_f_energy_consumption * task_energy_consumption_reward))
